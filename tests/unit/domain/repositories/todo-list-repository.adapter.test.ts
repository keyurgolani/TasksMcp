...
import type { TodoList, TodoListSummary } from '../../../../src/shared/types/todo.js';
import { TaskStatus as TaskStatusEnum, Priority as PriorityEnum } from '../../../../src/shared/types/todo.js';
...
class MockStorageBackend extends StorageBackend {
  ...
  async list(options?: { projectTag?: string; includeArchived?: boolean }): Promise<TodoListSummary[]> {
  ...
  async saveAllData(data: { version: string; todoLists: TodoList[] }): Promise<void> {
    this.data.clear();
...
describe('TodoListRepositoryAdapter', () => {
  ...
  describe('save', () => {
    ...
    it('should throw error if save fails', async () => {
      ...
      await expect(adapter.save(list)).rejects.toThrow('Failed to save TodoList list-1');
    });
  });
  ...
  describe('findById', () => {
    ...
    it('should throw error if load fails', async () => {
      ...
      await expect(adapter.findById('list-1')).rejects.toThrow('Failed to find TodoList list-1');
    });
  });
  ...
  describe('findAll', () => {
    ...
    it('should throw error if list operation fails', async () => {
      ...
      await expect(adapter.findAll()).rejects.toThrow('Failed to find all TodoLists');
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw error if delete fails', async () => {
      ...
      await expect(adapter.delete('list-1', true)).rejects.toThrow('Failed to delete TodoList list-1');
    });
  });
  ...
  describe('exists', () => {
    ...
    it('should throw error if check fails', async () => {
      ...
      await expect(adapter.exists('list-1')).rejects.toThrow('Failed to check if TodoList list-1 exists');
    });
  });
  ...
  describe('healthCheck', () => {
    ...
    it('should return false when health check throws error', async () => {
      vi.spyOn(storage, 'healthCheck').mockRejectedValue(new Error('Health check failed'));
