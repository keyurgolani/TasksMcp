...
console.log(`Found ${result.totalCount} lists across ${sources.length} sources`);
...
sortedResult.items.slice(0, 5).forEach((list, index) => {
  console.log(`  ${index + 1}. ${list.title} (updated: ${list.updatedAt.toISOString()})`);
});
console.log();
...
const paginatedResult = await aggregator.aggregateLists(sources, paginatedQuery);
...
console.log(`Pagination: offset=${paginatedResult.pagination?.offset}, limit=${paginatedResult.pagination?.limit}\n`);
...
console.log('  - In this example, "shared-db" (priority 2) wins over "local-storage" (priority 1)\n');
...
const summaryResult = await aggregator.aggregateSummaries(sources, summaryQuery);
