  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Step ${category}/${action} failed with exit code ${code}`));
      }
    });
...
async function runWorkflow(workflowName) {
  ...
  log(`${colors.bold}${colors.cyan}🚀 Running workflow: ${workflowName}${colors.reset}`);
  ...
  try {
    for (const [index, step] of workflow.steps.entries()) {
      ...
      log(`${colors.blue}[${stepNumber}/${totalSteps}] ${step.category}/${step.action}${colors.reset}`);
    ...
    log(`${colors.bold}${colors.green}🎉 Workflow '${workflowName}' completed successfully!${colors.reset}`);
    log(`${colors.green}Completed ${completedSteps}/${workflow.steps.length} steps in ${duration}s${colors.reset}`);
    
  } catch (error) {
    const duration = ((Date.now() - startTime) / 1000).toFixed(1);
    log(`${colors.bold}${colors.red}❌ Workflow '${workflowName}' failed!${colors.reset}`);
    log(`${colors.red}Completed ${completedSteps}/${workflow.steps.length} steps in ${duration}s${colors.reset}`);
...
// Show help
function showHelp() {
  log(`${colors.bold}${colors.cyan}MCP Task Manager - Development Workflows${colors.reset}\n`);
  ...
  Object.entries(WORKFLOWS).forEach(([name, config]) => {
    ...
    config.steps.forEach((step, index) => {
      const stepNumber = index + 1;
      log(`    ${colors.yellow}${stepNumber}.${colors.reset} ${step.category}/${step.action}`);
    });
    log('');
  });
