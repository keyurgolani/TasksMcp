...
export async function createTaskHandler(
  ...
  try {
    ...
    if (input.dependencies && input.dependencies.length > 0) {
      ...
      const invalidDeps = input.dependencies.filter(depId => !existingTaskIds.has(depId));
    ...
    if (input.implementationNotes !== undefined) {
      itemData.implementationNotes = input.implementationNotes;
    }
    ...
    if (!task) {
      throw new ApiError('INTERNAL_ERROR', 'Task not found after creation', 500);
    }
...
export async function updateTaskHandler(
  ...
  try {
    ...
    if (list.isArchived) {
      throw new ApiError('CONFLICT', 'Cannot update task in archived list', 409);
    }
    ...
    if (updates.dependencies && updates.dependencies.length > 0) {
      const existingTaskIds = new Set(list.items.map(item => item.id));
      const invalidDeps = updates.dependencies.filter(depId => !existingTaskIds.has(depId));
...
export async function deleteTaskHandler(
  ...
  try {
    ...
    if (list.isArchived) {
      throw new ApiError('CONFLICT', 'Cannot delete task from archived list', 409);
    }
...
export async function completeTaskHandler(
  ...
  try {
    ...
    if (list.isArchived) {
      throw new ApiError('CONFLICT', 'Cannot complete task in archived list', 409);
    }
    ...
    if (!updatedTask) {
      throw new ApiError('INTERNAL_ERROR', 'Task not found after completion', 500);
    }
