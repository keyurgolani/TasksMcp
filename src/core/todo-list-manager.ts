...
import type { StorageBackend, ListOptions } from '../types/storage.js';
...
import { FilteringUtils } from '../utils/filtering.js';
import { memoryCleanupManager, MemoryUtils } from '../utils/memory-cleanup.js';
import { memoryLeakDetector } from '../monitoring/memory-leak-detector.js';
...
export class TodoListManager {
  ...
  async updateTodoList(input: UpdateTodoListInput): Promise<TodoList> {
    try {
      ...
      switch (input.action) {
        case 'add_item':
          updatedItems = this.addItem(updatedItems, input.itemData, now);
          break;
    ...
    try {
      ...
      if (!summaries) {
        ...
        if (input.context !== undefined) {
          listOptions.context = input.context;
        }
