...
export class ErrorHandler extends EventEmitter {
  ...
  async executeWithRetry<T>(
    ...
    // Attempt operation with retry logic using exponential backoff
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        ...
        // Convert error to Error instance for consistent handling
        lastError = error instanceof Error ? error : new Error(String(error));

        // Update circuit breaker with failure information
        this.updateCircuitBreaker(context.operation, false);

        // Stop retrying if error is not retryable or we've exhausted attempts
