...
export class TodoListRepositoryAdapter implements ITodoListRepository {
  ...
  async findById(id: string, options?: FindOptions): Promise<TodoList | null> {
    try {
      ...
      if (options?.pagination) {
        filteredList = this.applyTaskPagination(filteredList, options.pagination);
      }
    ...
    try {
      ...
      if (query.text) {
        ...
        summaries = summaries.filter(
          summary => summary.title.toLowerCase().includes(searchText)
        );
      }
  ...
  private applyTaskFilters(list: TodoList, filters: TaskFilters): TodoList {
    const filteredItems = list.items.filter(item => {
      ...
      if (filters.estimatedDurationMin !== undefined && item.estimatedDuration) {
        if (item.estimatedDuration < filters.estimatedDurationMin) {
          return false;
        }
      }
      if (filters.estimatedDurationMax !== undefined && item.estimatedDuration) {
   ...
   */
  private applyTaskPagination(list: TodoList, pagination: PaginationOptions): TodoList {
    const offset = pagination.offset ?? 0;
   ...
   */
  private applyListSorting(lists: TodoList[], sorting: SortOptions): TodoList[] {
   ...
   */
  private applySummarySorting(summaries: TodoListSummary[], sorting: SortOptions): TodoListSummary[] {
